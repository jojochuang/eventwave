...
macedir = '/scratch/yoo7/mace-project'
ip_start = 10000
node = int(sys.argv[1])
...
for i in range(0,node):
	...
	if i == node-1:
		...
		print 'cd '+macedir+'/build/mace/application/appgroupstream; '+gdb_cmd+'./groupstream -ALL_NODES "' + allgroup + '"  -MACE_ADDRESS_ALLOW_LOOPBACK 1 -MACE_PORT '+str(ip_start + i*(num_transport*ip_interval)) + " -NUM_MESSAGES "+str(num_messages)+" -NUM_NODES "+str(node)+" -NUM_TRANSPORT "+str(num_transport)+" -IP_INTERVAL "+str(ip_interval)+" -DELAY $1 -PERIOD $2 -CURRENT "+str(current)+" 2>&1 | tee log_"+str(node)+"_"+str(num_transport)+"_$1_$2_" + str(i) + ".txt \n"
	else:
