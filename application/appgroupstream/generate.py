...
for i in range(0,node):
	...
	if i == node-1:
		...
		print 'mkdir -p '+macedir+'/build/mace/application/appgroupstream/'+str(i)+'; cd '+macedir+'/build/mace/application/appgroupstream/'+str(i)+'; rm ./*; '+gdb_cmd+'../groupstream -BOOTSTRAP_NODES "' + bootgroup + '" -ALL_NODES "' +allgroup + '" ' + logging_para + downgrade_to_none + ' -MACE_ADDRESS_ALLOW_LOOPBACK 1 -MACE_PORT '+str(ip_start + i*(num_transport*ip_interval)) + " -NUM_MESSAGES "+str(num_messages)+" -NUM_NODES "+str(node)+" -NUM_TRANSPORT "+str(num_transport)+" -IP_INTERVAL "+str(ip_interval)+" -DELAY $1 -PERIOD $2 -CURRENT "+str(current)+" 2>&1 | tee ../log_"+str(node)+"_"+str(num_transport)+"_$1_$2_" + str(i) + ".txt | egrep -v \"TRACE|DEBUG|BaseMaceService|ServiceStack|demux|routine|ThreadPool|DeliveryTransport|BaseTransport|TcpTransport|UdpTransport|SNAPSHOT|TcpConnection|expire_printer|AgentLock|Ticket\" \n"
	else:
